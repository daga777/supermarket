...
function getCostList(client, productsIds, quantities) {
  // Código aquí...
  //const productValues = {
  //  name:products[productsIds[0]],
  //  value:costs[productsIds[0]],
  //  qnty:quantity[productsIds[0]]
  //};
  ...
  productsIds.map(function (productToBuy, index) {
    const productValues = {
      name: products[productToBuy],
      value: costs[productToBuy],
      qnty: quantity[productToBuy],
      qntyToBuy: quantities[index],
    };
    //    storeStock;

    if (productValues.qnty >= productValues.qntyToBuy) {
